...
<div class="p-6 space-y-6" x-data="{ 
    ...
    handleNominalInput(index, val) {
        ...
        this.currentData.details[index].nominal = numeric ? parseInt(numeric) : 0;
        this.calculateTotal();
    },
    ...
    addDetail() {
        this.currentData.details.push({ nama_akun: '', nominal: 0 });
    },

    removeDetail(index) {
        if(this.currentData.details.length > 1) {
            this.currentData.details.splice(index, 1);
            this.calculateTotal();
        }
    },

    calculateTotal() {
        this.currentData.total_nominal = this.currentData.details.reduce((sum, item) => {
            return sum + (parseInt(item.nominal) || 0);
        }, 0);
    }
}">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 bg-white dark:bg-slate-900 p-4 rounded-3xl border border-slate-100 dark:border-white/5 shadow-sm">
        <div class="relative col-span-1 md:col-span-2">
            ...
            <input type="text" id="searchInput" placeholder="Cari kegiatan atau nama akun..." 
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div class="md:col-span-2 pt-4">
                            <div class="flex items-center justify-between mb-4 px-1">
                                <h4 class="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Rincian Per Akun</h4>
                                <button type="button" @click="addDetail()" class="flex items-center gap-1 text-[10px] font-black text-brand-primary uppercase hover:opacity-70 transition-opacity">
                            ...
                            <div class="space-y-3">
                                <template x-for="(detail, index) in currentData.details" :key="index">
                                    <div class="group flex flex-col md:flex-row gap-3 p-4 bg-slate-50 dark:bg-white/5 rounded-2xl border border-slate-200 dark:border-white/5 hover:border-brand-primary/50 transition-all">
                                        <div class="flex-1">
                                            <input type="text" :name="`details[${index}][nama_akun]`" x-model="detail.nama_akun" placeholder="Nama Akun Belanja..." required 
                                        ...
                                        <div class="w-full md:w-48">
                                            <input type="text" :value="formatRupiah(detail.nominal)" @input="handleNominalInput(index, $event.target.value)" placeholder="Rp 0" 
                                                class="w-full bg-white dark:bg-slate-800 border-none px-3 py-2.5 rounded-xl focus:ring-2 focus:ring-brand-primary outline-none text-sm font-black text-right text-brand-primary">
                                            <input type="hidden" :name="`details[${index}][nominal]`" :value="detail.nominal">
                                        </div>
                                        <button type="button" @click="removeDetail(index)" class="p-2.5 text-slate-300 hover:text-red-500 transition-colors">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-4v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                        </button>
                                    </div>
                                </template>
